...
import inspect
from numpy import array, linspace, log10, log, dtype,matrix
...
class funcFitter:
    ...
    def GetResultTab(self):
        ...
        fitVals= self.result[:,0]
        fitErrs = self.result[:,1]
        names = self.params
        #names.append["chi2"]
        #names.append["dof"]

        invert = (matrix(zip(fitVals[:-1],fitErrs[:,-1]))).transpose()
        
        tab = Table([fitVals[:-1],fitErrs[:-1]],names=names)
        tab.meta["chi2"] = fitVals[-1]
        tab.meta["dof"] = fitErrs[-1]
        tab.meta["model"] = self.funcName
        return tab
